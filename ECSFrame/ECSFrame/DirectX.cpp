...
RenderTarget* g_pRTV;
DepthStencil* g_pDSV;
ID3D11RasterizerState* g_pRasterizerState[CULL_MAX][FILL_MAX];
ID3D11BlendState* g_pBlendState[BLEND_MAX];
ID3D11SamplerState* g_pSamplerState[SAMPLER_MAX];

FillMode g_fillMode{FILL_SOLID};
CullMode g_cullMode{CULL_FRONT};
...
HRESULT InitDirectX(HWND hWnd, UINT width, UINT height, bool fullscreen)
{
	...
	for (UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes; ++driverTypeIndex)
	{
		...
		hr = D3D11CreateDeviceAndSwapChain(
			...
			D3D11_SDK_VERSION,		// DirectXバージョン
			&sd,					// スワップチェインの設定
	...
	for (int i = 0; i < CULL_MAX; ++i)
	{
		...
		{
			rasterizer.FillMode = fill[j];
			hr = g_pDevice->CreateRasterizerState(&rasterizer, &g_pRasterizerState[i][j]);
...
void UninitDirectX()
{
	...
	{
		for(int j = 0; j < FILL_MAX; ++j)
			SAFE_RELEASE(g_pRasterizerState[i][j]);
	}
	if(g_pContext)
...
{
	g_pContext->RSSetState(g_pRasterizerState[cull][g_fillMode]);
	g_cullMode = cull;
}
...
{
	g_pContext->RSSetState(g_pRasterizerState[g_cullMode][fill]);
	g_fillMode = fill;
}
